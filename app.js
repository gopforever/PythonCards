...
function renderChart() {
  ...
  if (!state.chart) {
    state.chart = new Chart(canvas, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { callbacks: { label: (ctx)=> `${ctx.dataset.label}: ${ctx.parsed.y.toLocaleString(undefined,{style:'currency',currency:'USD'})}` } }
        },
        scales: {
          x: { ticks: { color: "#9aa3b2" }, grid: { color: "rgba(255,255,255,.06)" } },
    ...
    state.chart.data.datasets[0].data = est;
    state.chart.data.datasets[1].data = cost;
    state.chart.update();
  }
}
...
function renderResults(products) {
  ...
  for (const p of products) {
    ...
    const priceRows = keys.map(k => `<div class="flex items-center justify-between text-sm">
...
function renderInventory() {
  ...
  if (!state.inventory.length) {
    list.innerHTML = `<div class="col-span-full text-slate-300/80">No items yet. Use search to add cards.</div>`;
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#toggle-history")?.addEventListener("click", () => {
    ...
    if (!card.classList.contains("hidden")) renderChart();
  });
});
