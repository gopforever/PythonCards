...
export default async (req) => {
  ...
  if (method === 'POST') {
    ...
    if (body.action === 'clear') { await client.setJSON(key, { unread: 0, items: [] }); return new Response(JSON.stringify({ ok:true }), { status: 200, headers: { 'content-type':'application/json' } }); }
    if (body.action === 'ack') { const data = (await client.getJSON(key)) || { unread:0, items:[] }; data.unread = 0; await client.setJSON(key, data); return new Response(JSON.stringify({ ok:true }), { status: 200, headers: { 'content-type':'application/json' } }); }
